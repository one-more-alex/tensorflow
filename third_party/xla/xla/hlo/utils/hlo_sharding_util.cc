...
Status CanonicalizeLayoutAfterShardingPropagation(
    HloModule* module, const bool update_output_layout,
    const bool update_parameters_layout) {
  if (!update_output_layout && !update_parameters_layout) {
    LOG(INFO) << "update_output_layout and update_parameters_layout are false";
    return OkStatus();
  }
