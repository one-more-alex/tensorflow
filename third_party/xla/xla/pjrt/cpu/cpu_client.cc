...
absl::StatusOr<PjRtLoadedExecutable::Result> TfrtCpuExecutable::ExecuteHelper(
  ...
  if (input_deps.empty() && execute_inline) {
    ...
    buffer_alloc.malloc();
    buffer_alloc_and_copy.malloc_and_copy();
    ...
    // `malloc_event` is used for delayed memory allocation. It must be ready
    // before `execute_event` is ready.
    auto malloc_event = tsl::MakeConstructedAsyncValueRef<CpuEvent>();
    malloc_event.AndThen(
        [buffer_alloc = std::move(buffer_alloc),
         buffer_alloc_and_copy = std::move(buffer_alloc_and_copy)]() mutable {
          buffer_alloc.malloc();
          buffer_alloc_and_copy.malloc_and_copy();
        });
    ...
    EnqueueWorkWhenReady(
        ...
        [cpu_executable, malloc_event = std::move(malloc_event),
         ...
         input_deps_avs = std::move(input_deps_avs_copy)]() mutable {
          ...
          // we are safe to set `malloc_event` ready. Since `execute_event` may
          // error out, we need to set `malloc_event` ready early.
