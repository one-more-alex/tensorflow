...
std::vector<AffineExpr> DimsToSymbols(std::vector<AffineExpr> expressions,
  ...
  // Move symbols right
  llvm::DenseMap<AffineExpr, AffineExpr> symbol_to_symbol_map;
  for (int i = 0; i < indexing_map.GetSymbolCount(); i++) {
    symbol_to_symbol_map[mlir::getAffineSymbolExpr(i, mlir_context)] =
        getAffineSymbolExpr(indexing_map.GetDimensionCount() + i, mlir_context);
  }
  for (AffineExpr& offset_expr : expressions) {
    offset_expr = offset_expr.replace(symbol_to_symbol_map);
  }
  ...
  for (AffineExpr& offset_expr : expressions) {
    offset_expr = offset_expr.replace(dim_to_symbol_map);
  }
...
mlir::AffineMap SymbolicTile::offset_map() const {
  ...
  CHECK_EQ(results.size() % 3, 0);
  llvm::ArrayRef<AffineExpr> offsets(results.begin(),
                                     results.begin() + results.size() / 3);
...
mlir::AffineMap SymbolicTile::size_map() const {
  llvm::ArrayRef<AffineExpr> results = tile_map_.GetAffineMap().getResults();
  CHECK_EQ(results.size() % 3, 0);
  llvm::ArrayRef<AffineExpr> offsets(results.begin() + results.size() / 3,
                                     results.begin() + results.size() / 3 * 2);
...
mlir::AffineMap SymbolicTile::stride_map() const {
  llvm::ArrayRef<AffineExpr> results = tile_map_.GetAffineMap().getResults();
  CHECK_EQ(results.size() % 3, 0);
  llvm::ArrayRef<AffineExpr> offsets(results.begin() + results.size() / 3 * 2,
                                     results.end());
  // RTVars are *not* included in the symbols.
  return AffineMap::get(
      /*dimCount=*/0,
